...
public class Game {
	...
	public static void main(String[] args) {
		...
		while (start_Game) { // 게임진행 루프
			...
			int p_hp = 0; // 플레이어 체력
			int p_hp_p = p_hp = 0; //체력 저장용
			int p_weapon = 0; // 플레이어 무기
			String weapon_name = ""; // 플레이어 무기 이름
...
//					System.out.println("체력이 너무 낮습니다. 휴식을 권합니다.");
				...
				case 1:
					...
					if (monster_encounter > 4) {
						...
						while (monster_attack) {
							int hit = rdm.nextInt(hit_dice);
							System.out.println("-----------------------------------------------------");
							System.out.println(hit + "가 나왔습니다.");

							if (hit > 5) {
								System.out.println("-----------------------------------------------------");
								System.out.println(p_name + "의 공격입니다.");
								p_attk = rdm.nextInt(weapon_dice) + p_str; // 플레이어 공격력
								System.out.println(p_name + "님이" + monster_name + "에게" + p_attk + "의 데미지를 입혔습니다.");
								...
								System.out.println(monster_name + "가" + p_name + "에게" + monster_attk + "의 데미지를 입혔습니다.");
